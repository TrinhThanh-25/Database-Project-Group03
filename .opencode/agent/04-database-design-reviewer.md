...
26. Authorized directives — DEPARTMENT + controlled-vocabulary lookup tables. These originate from explicit design directives in the agent instructions, NOT the source requirement — validate that the directive was applied and labeled as an assumption, not that the source stated it. Confirm the DEPARTMENT table exists (name + managing-user FK) and `USER_ACCOUNT.department_id` is an `INT` FK to it (no free-text department column remains). Confirm `role`/`account_status`/space `current_status`/`booking_status`/`maintenance_status` are realized as lookup tables with `INT` FKs (not string+CHECK). Flag a missing lookup table, a residual string+CHECK for a closed vocabulary, or a missing DEPARTMENT normalization as Medium severity.
...
> **Phase 1 note:** This project is currently Phase 1 (schema definition without triggers/indexes). Rules classified as "requires implementation logic" (overlap, role checks, completion consistency, rejection reason) are **deferred to Phase 2** and are expected to appear only as `-- PHASE 2` comment blocks in the DDL. Their absence as executable triggers is **not** a defect in Phase 1 — mark them "Deferred to Phase 2" in the *Enforced in DDL* column, not as a High-severity gap. The absence of manual FK/performance indexes is likewise not a defect in Phase 1.
