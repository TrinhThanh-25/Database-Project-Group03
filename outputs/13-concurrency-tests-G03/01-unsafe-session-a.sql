/* Window A: start first. Deliberately naive test code; never a production path. */
SET NOCOUNT ON; SET XACT_ABORT ON;
DECLARE @S INT=(SELECT space_id FROM dbo.SPACE WHERE unique_space_code=N'G03-CT-UNSAFE'),@U INT=(SELECT user_account_id FROM dbo.USER_ACCOUNT WHERE user_id=N'G03-CT-USER-A'),@A INT=(SELECT booking_status_id FROM dbo.BOOKING_STATUS WHERE status_code=N'approved');
BEGIN TRANSACTION;
